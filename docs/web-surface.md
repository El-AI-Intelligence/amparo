...
Status: **contract v1** (2026-08-30). This document is the binding
...
- **`--approval-endpoint`** (ships in v0.9.0; see §3) replaces the
  ...
  `--auto-deny` (usage error otherwise).
...
timeout, fail closed**: no decision by then is a denial. Both fields
(`blast_radius`, `session_label`) are display-only by invariant I1 —
they never feed back into the gate decision.
