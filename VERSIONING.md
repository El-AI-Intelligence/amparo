...
- **0.12.0 (2026-09-05)** — the coding surface: `amparo code` opens a
  directory as an interactive editor (file tree with git marks,
  diff-accept edits behind the same gate chain as `amparo run`, a
  streaming build/shell pane; piped = report), CI on three platforms at
...
- **0.14.0 (2026-09-08)** — Windows interactive parity: `amparo tui`
  and `amparo code` run full-screen on Windows (Windows Terminal and the
